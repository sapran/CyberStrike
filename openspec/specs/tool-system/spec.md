...
- **THEN** Returns an ordered array: InvalidTool, QuestionTool (if client is app/cli/desktop), then 43 built-in tools (BashTool through CipipeTool), then custom/plugin tools (registry.ts:133-194)
...
- The LazyToolRegistry auto-categorizes MCP tools into 8 categories (web, network, file, security, cloud, database, git, shell, general) via keyword matching against tool names and descriptions (lazy-registry.ts:379-400).
