...
The system SHALL require only `name` and `description` in frontmatter, treat all signing, categorization, tech-stack and kill-chain fields as optional, and silently skip files whose frontmatter fails to parse or lacks the required pair.
...
- `skills.disabled` is defined in config (config.ts:756) but is NOT enforced by the skill tool or loader — only the HTTP server routes honor it (packages/cyberstrike/src/server/routes/skill.ts:338,371). An agent using the `skill` tool can load a skill listed in skills.disabled.
