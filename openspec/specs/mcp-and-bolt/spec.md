...
- Timeout discrepancy: the config schema descriptions state 'Defaults to 5000 (5 seconds)' (config.ts:606,645,664) but the actual runtime default is DEFAULT_TIMEOUT = 30_000ms (index.ts:33). The doc string is stale; effective default is 30s for both MCP and Bolt.
- LazyTool.source is typed as 'mcp'|'plugin'|'builtin' (lazy-registry.ts:30) but init/refresh only ever set source:'mcp' — plugin/builtin lazy sourcing is not wired. Both regular MCP and Bolt clients funnel through the same registry (init merges mcpStatus + boltStatus at lazy-registry.ts:49-51).
...
- The '~50 token' metadata vs '~500 token full definition' figures come from the module's own doc comment and constant AVG_TOKENS_PER_TOOL=500 (lazy-registry.ts:15-20,44); they are hardcoded estimates, not measured — the budget-exceeded path only warns, it does not actually evict tools (lazy-registry.ts:203-210).
